...
public class PinnedCategorySectionListAdapter extends BaseAdapter implements PinnedSectionListView.PinnedSectionListAdapter, SectionIndexer {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null)
            convertView = inflater.inflate(R.layout.service_list_item_new, null);
        ...
        LinearLayout serviceImageArea =
                (LinearLayout) convertView.findViewById(R.id.serviceImageArea);
        ImageView serviceImage = (ImageView) convertView.findViewById(R.id.serviceImage);
...
//			convertView.setAlpha((float) 0.96);
          ...
          //  headingTitleTxt.setPaddingRelative(padding, padding, padding1, padding1);
            ...
            descTxt.setText(AppFunctions.fromHtml(Utils.checkText(vShortDesc)?vShortDesc:categoryListItem.getvDesc()));
            ...
            if (categoryListItem.isAdd) {
                ...
                btnArea.setBackground(mContext.getResources().getDrawable(R.drawable.remove_border));
                if (generalFunctions.isRTLmode()) {
                    btnImg.setRotation(180);
                    btnArea.setBackground(mContext.getResources().getDrawable(R.drawable.remove_border_rtl));

                }
            } else {
                btnTxt.setText(LBL_BOOK_SERVICE);
                btnArea.setBackground(mContext.getResources().getDrawable(R.drawable.login_border));
                if (generalFunctions.isRTLmode()) {
                    btnImg.setRotation(180);
...
//			txt_count.setVisibility(View.GONE);
        ...
        String vServiceImage = categoryListItem.getvServiceImage();

        if(vServiceImage.equals("")){
            serviceImageArea.setVisibility(View.GONE);
        }else{
            serviceImageArea.setVisibility(View.VISIBLE);
            Picasso.get()
                    .load(categoryListItem.getvServiceImage())
