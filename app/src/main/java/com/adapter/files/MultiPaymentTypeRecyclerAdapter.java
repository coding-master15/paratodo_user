...
import com.paratodo.user.MultiDeliveryThirdPhaseActivity;
...
public class MultiPaymentTypeRecyclerAdapter extends RecyclerView.Adapter<MultiPaymentTypeRecyclerAdapter.ViewHolder> implements MultiListViewAdapter.OnItemClickListener {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (holder.radioBtn.isChecked() && position==2) {
            ...
            holder.typeInstTxt.setText("" + LBL_RECIPIENT_PAY_INST_TXT + " " + new AppFunctions(mContext).formatNumAsPerCurrency(generalFunc, ((MultiDeliveryThirdPhaseActivity) mActivity).individualFare,((MultiDeliveryThirdPhaseActivity) mActivity).currencySymbol,true)+ " "+ generalFunc.retrieveLangLBl("", "LBL_AMOUNT"));
        } else {
