...
public class ProfilePaymentActivity extends AppCompatActivity {
    ...
    String selCardPayment = "";
    public static final int REQUEST_CODE = 11;
    private String vCyberSourceToken;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        commentBox.setPadding(10, 5, 0, 5);
...
//            cardTxt.setText(generalFunc.retrieveLangLBl("", "LBL_PAY_BY_WALLET_TXT"));
            ...
            userWalletArea.setVisibility(View.GONE);
            walletView.setVisibility(View.GONE);
            cardImg.setImageResource(R.drawable.ic_credit_card);
    ...
    public void manageView() {
        ...
        if (getIntent().getBooleanExtra("isRide", false)) {
            ...
            if (ePaymentBy.equalsIgnoreCase("Passenger") || !ePaymentBy.equalsIgnoreCase("Organization")) {
                ...
                {
                    selCurrentPosition= getIntent().getIntExtra("selectPos",0);
                }
                if (!vReasonTitle_.equalsIgnoreCase("")) {
                    //commentarea.setVisibility(View.VISIBLE);
//                    reasonArea.setVisibility(View.GONE);
...
//                cardValArea.setVisibility(View.GONE);
            ...
            String eTakeAway = getIntent().getStringExtra("eTakeAway");
            isPreference = getIntent().getBooleanExtra("isPreference", false);
            boolean isTakeAwayOrPreference = (Utils.checkText(eTakeAway) && eTakeAway.equalsIgnoreCase("Yes")) || isPreference;
            if ((!getIntent().getBooleanExtra("isCODAllow", true) && !getIntent().getBooleanExtra("isGenie", false)) || isTakeAwayOrPreference) {
                codNotAllowTxt.setText(generalFunc.retrieveLangLBl("", generalFunc.retrieveLangLBl("", isTakeAwayOrPreference ? "LBL_COD_DISABLED_TXT" : "LBL_COD_NOT_AVAILABLE_TXT")) + " " + getIntent().getStringExtra("outStandingAmount"));
    ...
    public void manageCashCardView(boolean isCash) {
        if (isCash) {
            ...
            isPreference = getIntent().getBooleanExtra("isPreference", false);
            boolean isTakeAwayOrPreference = (Utils.checkText(eTakeAway) && eTakeAway.equalsIgnoreCase("Yes")) || isPreference;
            if (!getIntent().getBooleanExtra("isCODAllow", true) || isTakeAwayOrPreference) {
                cashArea.setBackgroundColor(getActContext().getResources().getColor(R.color.gray));
            }
            ...
            if ((SYSTEM_PAYMENT_FLOW.equalsIgnoreCase("method-1") && !selectedMethod.equalsIgnoreCase("Instant")) || (SYSTEM_PAYMENT_FLOW.equalsIgnoreCase("method-1") && Utils.checkText(selectedMethod) && selectedMethod.equalsIgnoreCase("Manual"))) {
                ...
                if (!vCreditCard.equalsIgnoreCase("")&& (APP_PAYMENT_MODE.equalsIgnoreCase("Card")||APP_PAYMENT_MODE.equalsIgnoreCase("Cash-Card"))) {
            ...
            if (selCardPayment.equalsIgnoreCase("wallet") || selCardPayment.equalsIgnoreCase("")) {
                cardValArea.setVisibility(View.GONE);
            }
        }
    }
...
//        getUserProfileJson();
        ...
        isPreference = getIntent().getBooleanExtra("isPreference", false);
        boolean isTakeAwayOrPreference = (Utils.checkText(eTakeAway) && eTakeAway.equalsIgnoreCase("Yes")) || isPreference;
        if (isTakeAwayOrPreference) {
            codNotAllowTxt.setText(generalFunc.retrieveLangLBl("", generalFunc.retrieveLangLBl("COD not available as you have selected Preferences.", "LBL_COD_DISABLED_TXT")) + " " + getIntent().getStringExtra("outStandingAmount"));
            if (!APP_PAYMENT_MODE.equalsIgnoreCase("Card")) {
                codNotAllowTxt.setVisibility(View.VISIBLE);
            }
        }
    ...
    public void setLabel() {
        ...
        demoText.setText(generalFunc.retrieveLangLBl("", "LBL_CARD_PAYMENT_NOTE"));
...
/*
                ...
                OpenListView.getInstance(getActContext(), generalFunc.retrieveLangLBl("", "LBL_SELECT_PROFILE"), listdata, OpenListView.OpenDirection.CENTER, true, position -> {
                    ...
                    if(filterReasonlistdata!=null && filterReasonlistdata.size()>0 && !vProfileName.equalsIgnoreCase(LBL_PERSONAL)) {
                        vReasonName = filterReasonlistdata.get(0).get("vReasonTitle");
                        selreasonBoxTxt.setText(filterReasonlistdata.get(0).get("vReasonTitle"));
                        selectReasonId = filterReasonlistdata.get(0).get("iTripReasonId");
                        selCurrentPositionorg = 0;
                    }



                }).show(selCurrentPosition, "vProfileName");
                Logger.d("selCurrentPosition","::"+selCurrentPosition);
                ...
                for (int j = 0; j < reasonlistdata.size(); j++) {
                    HashMap<String, String> posData ;
                    posData = reasonlistdata.get(j);
 ...
 /*
                ...
                OpenListView.getInstance(getActContext(), generalFunc.retrieveLangLBl("", "LBL_SELECT_REASON"), filterReasonlistdata, OpenListView.OpenDirection.CENTER, true, position -> {
                ...
                if (selectReasonId.equalsIgnoreCase("-1") && commentBox.getText().toString().length() == 0) {
                    generalFunc.showMessage(commentarea, generalFunc.retrieveLangLBl("", "LBL_RESTRICT_ADD_REASON"));
                    return;
                ...
                if (!isCashSelect && selCardPayment.equalsIgnoreCase("")) {
                    OpenPaymentcardOption(false);
                    return;
                }

                if (!selCardPayment.equalsIgnoreCase("") && selCardPayment.equalsIgnoreCase("wallet")) {
                    handleButton("");
                    return;
                }

                if (!selCardPayment.equalsIgnoreCase("") && selCardPayment.equalsIgnoreCase("CyberSource")) {
                    OpenCardPaymentAct(true);
                    return;
                }


                if ((APP_PAYMENT_MODE.equalsIgnoreCase("Card") || !isCashSelect) && !ePaymentBy.equalsIgnoreCase("Organization")/*&&  !generalFunc.isDeliverOnlyEnabled()*/ && !APP_PAYMENT_MODE.equalsIgnoreCase("Cash") && SYSTEM_PAYMENT_FLOW.equalsIgnoreCase("method-1")) {


                    String vCreditCard = generalFunc.getJsonValue("vCreditCard", userProfileJson);

                    if (selCardPayment.equalsIgnoreCase("CyberSource") && vCreditCard.equals("")) {
                        selectCardTxt.performClick();
                        return;
                    }
    ...
    public void checkCardConfig() {
        ...
        if (APP_PAYMENT_METHOD.equalsIgnoreCase("Stripe")) {
            ...
            if (vCreditCard.equals("") && vBrainTreeCustEmail.equalsIgnoreCase("")) {
                ...
                APP_PAYMENT_METHOD.equalsIgnoreCase("CyberSource") ||
        ...
        *//*Flutterwave Payment GateWay*//*
        else if (APP_PAYMENT_METHOD.equalsIgnoreCase("Flutterwave") && !generalFunc.isDeliverOnlyEnabled()) {
...
//        bn.putBoolean("fromcabselection", fromcabselection);
        bn.putBoolean("isOpen", true);
        bn.putString("selCardPayment", selCardPayment);
        String amount = "";
        if (getIntent().hasExtra("Amount")) {
            amount = getIntent().getStringExtra("Amount");
        }
        if(!vCyberSourceToken.equalsIgnoreCase("")){

            bn.putString("Amount", getIntent().getStringExtra("Amount"));
            bn.putString("OnlyCVV", "true");
            bn.putString("NewCard", "false");
            bn.putString("token", vCyberSourceToken);
        }else {

            bn.putString("Amount", getIntent().getStringExtra("Amount"));
            bn.putString("OnlyCVV", "false");
            bn.putString("NewCard", "false");
            bn.putString("token", vCyberSourceToken);
        }
        new StartActProcess(getActContext()).startActForResult(CyberSourceAddCardActivity.class, bn, REQUEST_CODE);
...
//            selectReasonId = "";
    ...
    public void showPaymentBox(boolean isOutstanding, boolean isReqNow) {
        ...
        final TextInputEditText input = (TextInputEditText) dialogView.findViewById(R.id.editBox).findViewWithTag("edittext");
...
//                        }
    ...
    public void handleButton(String isCash) {
        Bundle bundle = new Bundle();
        if (selectPos == 0) {
            bundle.putSerializable("data", "");


            if (isCashSelect || APP_PAYMENT_MODE.equalsIgnoreCase("Cash")) {
                if(isCash.equalsIgnoreCase("no"))
                {
        ...
        bundle.putString("selCardPayment", selCardPayment);
    ...
    private void getUserProfileJson() {
        ...
        vCyberSourceToken = generalFunc.getJsonValue("vCyberSourceToken", userProfileJson);
        vCreditCard = generalFunc.getJsonValue("vCreditCard", userProfileJson);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (APP_PAYMENT_METHOD.equalsIgnoreCase("CyberSource")) {
            String vCreditCard = generalFunc.getJsonValue("vCreditCard", userProfileJson);
            isCardNotAdded = vCreditCard.equals("");
        }

        if (requestCode == REQUEST_CODE) {
            if(data!=null) {
                if (data.getStringExtra("message").equals("success")) {
...
//                        generalFunc.storeData(Utils.USER_PROFILE_JSON, generalFunc.getJsonValue(Utils.message_str, responseString));
                        if (!selCardPayment.equalsIgnoreCase("") && selCardPayment.equalsIgnoreCase("CyberSource")) {
                            handleButton("no");
                        }else if(!selCardPayment.equalsIgnoreCase("") && selCardPayment.equalsIgnoreCase("wallet")){
                            handleButton("no");
                        }
                    ...
                    generalFunc.showGeneralMessage("", data.getStringExtra("message"), "", generalFunc.retrieveLangLBl("", "LBL_OK"), i -> {
//                        isFinish = true;
    ...
    public void OpenPaymentcardOption(boolean iscardclick) {
        ...
        MTextView cancelTxt = dialog.findViewById(R.id.cancelTxt);
        MTextView stripeTxt = dialog.findViewById(R.id.stripeTxt);
        MTextView paypalTxt = dialog.findViewById(R.id.paypalTxt);
        MTextView walletBalanceTxt = dialog.findViewById(R.id.walletBalanceTxt);
        MTextView paypalNoteTxt = dialog.findViewById(R.id.paypalNoteTxt);
        LinearLayout paypalArea = dialog.findViewById(R.id.paypalArea);
        cancelTxt.setText(generalFunc.retrieveLangLBl("", "LBL_CANCEL_TXT"));
        stripeTxt.setText(generalFunc.retrieveLangLBl("via Stripe", "LBL_PAY_VIA_CARD"));
        paypalTxt.setText(generalFunc.retrieveLangLBl("", "LBL_PAY_BY_WALLET_TXT"));
        paypalNoteTxt.setText(generalFunc.retrieveLangLBl("", "LBL_AMOUNT_DEDUCT_FROM_WALLET_NOTE"));
        String userBal = generalFunc.convertNumberWithRTL(generalFunc.getJsonValue("user_available_balance", userProfileJson));
        walletBalanceTxt.setText("(" + userBal + ")");
        vCyberSourceToken = generalFunc.getJsonValue("vCyberSourceToken", userProfileJson);
        ...
        stripeTxt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selCardPayment = "CyberSource";
                if (iscardclick) {
                    vCreditCard = generalFunc.getJsonValue("vCreditCard", userProfileJson);

                    if (vCyberSourceToken.equals("")) {
                        OpenCardPaymentAct(true);
                    } else {
                        cardValArea.setVisibility(View.VISIBLE);
        ...
        paypalArea.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selCardPayment = "wallet";
        ...
        cancelTxt.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (iscardclick && codNotAllowTxt.getVisibility() == View.GONE) {
                    // manageCashCardView(true);
                }
                dialog.dismiss();
