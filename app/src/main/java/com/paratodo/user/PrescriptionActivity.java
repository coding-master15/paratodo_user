...
//                        generalFunc.retrieveLangLBl("", generalFunc.getJsonValue(Utils.message_str, responseString)));
    ...
    public void onItemClickList(View v, int position) {
        ...
        if (position == 0 && !getIntent().getBooleanExtra("isOrder", false)) {
            ...
            //carouselView.setSize(listData.size() - 1);
...
//        layParams.rightMargin = Utils.dipToPixels(getActContext(), 15);
        ...
        try {
            Picasso.get()
                    .load(Utils.getResizeImgURL(getActContext(), item.get("vImage"), ((int) Utils.getScreenPixelWidth(getActContext())) - Utils.dipToPixels(getActContext(), 30), 0, Utils.getScreenPixelHeight(getActContext()) - Utils.dipToPixels(getActContext(), 30)))
