...
//        }
    ...
    private void initView() {
        ...
        skeletonScreen = SkeletonLayoutUtils.createSkeleton(mainArea, new SkeletonConfig(
                ContextCompat.getColor(this, R.color.skeleton_mask),
                SkeletonLayout.DEFAULT_MASK_CORNER_RADIUS,
                SkeletonLayout.DEFAULT_SHIMMER_SHOW,
                ContextCompat.getColor(this, R.color.default_shimmer_color),
        ...
        CommonDeliveryTypeSelectionActivity.MyHandler myHandler = new CommonDeliveryTypeSelectionActivity.MyHandler(this);
        myHandler.sendEmptyMessageDelayed(1, 5000);

    }

    public static class MyHandler extends android.os.Handler {
        private final WeakReference<CommonDeliveryTypeSelectionActivity> activityWeakReference;

        MyHandler(CommonDeliveryTypeSelectionActivity activity) {
            ...
            if (responseString != null && !responseString.equals("")) {
                ...
                if (isDataAvail) {
                    ...
                    if (generalFunc.retrieveValue("CHECK_SYSTEM_STORE_SELECTION").equalsIgnoreCase("Yes")) {
                        bn.putString("iCompanyId", generalFunc.getJsonValue("iCompanyId",userProfileJson));
                        bn.putString("ispriceshow", generalFunc.getJsonValue("ispriceshow",userProfileJson));
                        bn.putString("eAvailable", generalFunc.getJsonValue("eAvailable",userProfileJson));
                        bn.putString("ispriceshow", generalFunc.getJsonValue("ispriceshow",userProfileJson));
                        bn.putString("timeslotavailable", generalFunc.getJsonValue("timeslotavailable",userProfileJson));
