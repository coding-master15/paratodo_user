...
import com.paratodo.user.AdditionalChargeActivity;
import com.paratodo.user.BookingActivity;
import com.paratodo.user.CommonDeliveryTypeSelectionActivity;
import com.paratodo.user.HistoryDetailActivity;
import com.paratodo.user.MoreServiceInfoActivity;
import com.paratodo.user.OnGoingTripDetailsActivity;
import com.paratodo.user.R;
import com.paratodo.user.RideDeliveryActivity;
import com.paratodo.user.ScheduleDateSelectActivity;
import com.paratodo.user.UberXHomeActivity;
...
import com.paratodo.user.VerifyServiceActivity;
...
public class NewBookingFragment extends Fragment implements AllHistoryRecycleAdapter.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        nestedScrollView.setOnScrollChangeListener((NestedScrollView.OnScrollChangeListener) (v, scrollX, scrollY, oldScrollX, oldScrollY) -> {
            ...
            if (getChild != null) {
                if ((scrollY >= (getChild.getMeasuredHeight() - v.getMeasuredHeight())) &&
                        scrollY > oldScrollY) {
    ...
    public void onItemClickList(View v, int position, String type) {
        ...
        if (type.equalsIgnoreCase("ShowDetail")) {
            ...
            bn.putString("tMaterialFee", listData.get("tMaterialFee"));
            bn.putString("tMiscFee", listData.get("tMiscFee"));
            bn.putString("vSatisfactionBeforeImage", listData.get("vSatisfactionBeforeImage"));
            bn.putString("vSatisfactionAfterImage", listData.get("vSatisfactionAfterImage"));
            ...
            new StartActProcess(getActivity()).startActWithData(AdditionalChargeActivity.class, bn);
        }
        else if (type.equalsIgnoreCase("VerifyServiceEndApproval")) {
            Bundle bn = new Bundle();
            bn.putSerializable("TripDetail", listData);
            bn.putString("iTripId", listData.get("iTripId"));
            bn.putString("iDriverId", listData.get("iDriverId"));
            ...
            bn.putString("fDriverDiscount", listData.get("fDriverDiscount"));
            bn.putString("vConfirmationCode", listData.get("vConfirmationCode"));
            bn.putString("eVerifyRequestSentByDriver", listData.get("eVerifyRequestSentByDriver"));
            bn.putString("eApproveRequestSentByDriver", listData.get("eApproveRequestSentByDriver"));
            bn.putString("CurrencySymbol", listData.get("CurrencySymbol"));
            bn.putBoolean("isnotification", false);
            bn.putString("isFromToll", "No");
//            showAlertVerify(bn);
            new StartActProcess(getActivity()).startActWithData(VerifyServiceActivity.class, bn);
        }
    ...
    public void onViewServiceClickList(View v, int position) {
        ...
        bn.putString("iCabBookingId", listData.get("iCabBookingId"));
        new StartActProcess(getActivity()).startActWithData(MoreServiceInfoActivity.class, bn);
...
//        bundle.putString("address", posData.get("vSourceAddresss"));
    ...
    public void showDeclineReasonsAlert(String responseString) {
        ...
        TextInputEditText reasonBox = (TextInputEditText) dialogView.findViewById(R.id.inputBox).findViewWithTag("edittext");
        ...
        if (generalFunc.isRTLmode()) {
            reasonBox.setPadding(0, 0, size10sdp, 0);
        } else {
            reasonBox.setPadding(size10sdp, 0, 0, 0);
        }
        ...
        reasonBox.setHint(generalFunc.retrieveLangLBl("", "LBL_ENTER_REASON"));
        ...
        new SlideDateTimePicker.Builder(requireActivity().getSupportFragmentManager())
                .setListener(new SlideDateTimeListener() {
                    ...
                    public void onDateTimeSet(Date date) {
                        ...
                        int minHours = Integer.parseInt(generalFunc.getJsonValue("MINIMUM_HOURS_LATER_BOOKING", userProfileJson));
                        int maxHours = Integer.parseInt(generalFunc.getJsonValue("MAXIMUM_HOURS_LATER_BOOKING", userProfileJson));

                        if (!Utils.isValidTimeSelect(date, TimeUnit.HOURS.toMillis(minHours)) || Utils.isValidTimeSelect(date, TimeUnit.HOURS.toMillis(maxHours))) {

                            String msg = generalFunc.retrieveLangLBl("", "LBL_INVALID_PICKUP_MAX_NOTE_MSG");
                            String newMsg = msg.replace("#####", maxHours + " " + generalFunc.retrieveLangLBl("hour", "LBL_HOUR_TXT"));
                            newMsg = newMsg.replace("####", minHours + " " + generalFunc.retrieveLangLBl("hour", "LBL_HOUR_TXT"));

                            generalFunc.showGeneralMessage(generalFunc.retrieveLangLBl("Invalid pickup time", "LBL_INVALID_PICKUP_TIME"), newMsg);
                            return;
                        }
...
//        Logger.d("RESPONSE_TRACKING", "New Booking 1 >>" + System.currentTimeMillis());
...
//        Logger.d("RESPONSE_TRACKING", "New Booking 2 >>" + System.currentTimeMillis());
...
//        Logger.d("RESPONSE_TRACKING", "New Booking 3 >>" + System.currentTimeMillis());

        final ExecuteWebServerUrl exeWebServer = new ExecuteWebServerUrl(getActivity(), parameters);
        exeWebServer.setDataResponseListener(responseString -> {
//            Logger.d("RESPONSE_TRACKING", "New Booking 4 >>" + System.currentTimeMillis());
            ...
            swipeRefreshLayout.setRefreshing(false);
//            Logger.d("RESPONSE_TRACKING", "New Booking 5 >>" + System.currentTimeMillis());

            if (responseObj != null && !responseObj.equals("")) {
//                Logger.d("RESPONSE_TRACKING", "New Booking 6 >>" + System.currentTimeMillis());
...
//                    Logger.d("RESPONSE_TRACKING", "New Booking 7 >>" + System.currentTimeMillis());
                    ...
                    if (arr_rides != null) {
                        ...
                        if (arr_rides.length() > 0) {
                            ...
                            for (int i = 0; i < arrRidesSize; i++) {
                                ...
                                map.put("vAvgRating", "" + generalFunc.parseFloatValue(0, Utils.checkText(vAvgRating) ? vAvgRating : ""));
                                ...
                                map.put("eVerifyRequestSentByDriver", generalFunc.getJsonValueStr("eVerifyRequestSentByDriver", obj_temp));
                                map.put("eSatisfactionApproved", generalFunc.getJsonValueStr("eSatisfactionApproved", obj_temp));
                                map.put("verifyServiceLBL", generalFunc.retrieveLangLBl("", "LBL_SATISFICATION_TITLE_TXT"));
                                ...
                                map.put("tMaterialFee", vChargesDetailDataObj != null ?
                                        generalFunc.getJsonValueStr("tMaterialFee", vChargesDetailDataObj) : "");
                                map.put("tMiscFee", vChargesDetailDataObj != null ? generalFunc.getJsonValueStr("tMiscFee", vChargesDetailDataObj) : "");

                                map.put("vSatisfactionBeforeImage", vChargesDetailDataObj != null ?
                                        generalFunc.getJsonValueStr("vSatisfactionBeforeImage", vChargesDetailDataObj) : "");
                                map.put("vSatisfactionAfterImage", vChargesDetailDataObj != null ? generalFunc.getJsonValueStr("vSatisfactionAfterImage", vChargesDetailDataObj) : "");
...
//                    Logger.d("RESPONSE_TRACKING", "New Booking 7 >>" + System.currentTimeMillis());
    ...
    public void showAlertVerify(Bundle bn){
        ...
        messageTxt.setText(generalFunc.retrieveLangLBl("", "LBL_CONFIRM_SERVICE_END_NOTE"));
        submitTxt.setText(generalFunc.retrieveLangLBl("", "LBL_APPROVE_TXT"));
        cancelTxt.setText(generalFunc.retrieveLangLBl("", "LBL_DECLINE_TXT"));
        cancelTxt.setOnClickListener(view -> {
            if (alertDialog != null) {
                alertDialog.dismiss();
            }
            sendChargeVerificationCode("No", bn);
        });
        submitTxt.setOnClickListener(view -> {
            if (alertDialog != null) {
                alertDialog.dismiss();
            }
            sendChargeVerificationCode("Yes", bn);
    ...
    private void sendChargeVerificationCode(String eApproveByUser, Bundle bn) {
        HashMap<String, String> parameters = new HashMap<String, String>();
        parameters.put("type", "sendVerificationApproval");
        parameters.put("TripId", bn.get("iTripId").toString());
        parameters.put("iUserId", generalFunc.getMemberId());
        parameters.put("UserType", Utils.app_type);
