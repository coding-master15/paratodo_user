...
import com.paratodo.user.SearchLocationActivity;
import com.paratodo.user.SearchServiceActivity;
import com.paratodo.user.UberXHomeActivity;
import com.paratodo.user.UberXSelectServiceActivity;
...
public class HomeDaynamicFragment extends Fragment implements UberXCategoryAdapter.OnItemClickList, ViewPager.OnPageChangeListener
    ...
    private Skeleton skeletonScreen;
    ImageView arrowImg;
...
//    private ImageView imageCancel;
    ...
    private ArrayList<HashMap<String, String>> searchCategoryList = new ArrayList<>();
...
//        loaderView.setVisibility(View.GONE);
        ...
        searchTxtView = (EditText) view.findViewById(R.id.searchTxtView);
        searchTxtView.setHint(generalFunc.retrieveLangLBl("", "LBL_SEARCH_SERVICES"));
        searchTxtView.setOnClickListener(new setOnClickLst());
...
//            }
        ...
        skeletonScreen = SkeletonLayoutUtils.createSkeleton(MainLayout, new SkeletonConfig(
                ContextCompat.getColor(getActContext(), R.color.skeleton_mask),
                SkeletonLayout.DEFAULT_MASK_CORNER_RADIUS,
                SkeletonLayout.DEFAULT_SHIMMER_SHOW,
                ContextCompat.getColor(getActContext(), R.color.default_shimmer_color),
    ...
    public static class MyHandler extends android.os.Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (activityWeakReference.get() != null) {
                activityWeakReference.get().skeletonScreen.showOriginal();
            }
        }
    }
    ...
    public void onItemClickHandle(int position, String type) {
        ...
        if (type.equalsIgnoreCase("HOME")) {
            ...
            if (searchCategoryList.size() > 0) {
                mapData = searchCategoryList.get(position);
            } else {
...
//        }
    ...
    public void setSubCategoryList(HashMap<String, String> dataItem) {
        ...
        skeletonScreen = SkeletonLayoutUtils.createSkeleton(MainArea, new SkeletonConfig(
                ContextCompat.getColor(getActContext(), R.color.skeleton_mask),
                SkeletonLayout.DEFAULT_MASK_CORNER_RADIUS,
                SkeletonLayout.DEFAULT_SHIMMER_SHOW,
                ContextCompat.getColor(getActContext(), R.color.default_shimmer_color),
...
//                    bn.putBoolean("isaddressview", true);
    ...
    public void onCategorySelect(int pos) {
        ...
        HashMap<String, String> mapData = null;
        mapData = filtergeneralCategoryList.get(pos);
        String eCatType = mapData.get("eCatType");
        Logger.d("E_CAT_TYPE", "::::"   + eCatType);
        if (eCatType.equalsIgnoreCase("ServiceProvider")) {
            if (generalFunc.getJsonValueStr("ENABLE_SEARCH_UFX_SERVICES", obj_userProfile).equalsIgnoreCase("YES")) {
                searchView.setVisibility(View.VISIBLE);
...
//                    bn.putBoolean("isaddressview", true);
                ...
                case R.id.searchTxtView:
                    ...
                    bn.putString("address", headerLocAddressTxt.getText().toString());
                    bn.putString("SelectvVehicleType", vParentCategoryName);
                    String SelectedVehicleTypeId = "";
                    if (multiServiceSelect.size() > 0) {
                    ...
                    bn.putString("parentId", parentId);
                    bn.putBoolean("isCarwash", true);
                    new StartActProcess(getActContext()).startActWithData(SearchServiceActivity.class, bn);

//                    moreDialogSearchView();
    ...
    BottomSheetDialog moreDialogSearchView;

    public void moreDialogSearchView() {
        if (moreDialogSearchView != null && moreDialogSearchView.isShowing()) {
            return;
        }
        moreDialogSearchView = new BottomSheetDialog(getActContext());
        searchCategoryList.clear();

        View contentView = View.inflate(getActContext(), R.layout.dialog_more_new, null);
        if (generalFunc.isRTLmode()) {
            contentView.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);
        }
        moreDialogSearchView.setContentView(contentView);

        moreDialogSearchView.setCancelable(true);
        BottomSheetBehavior mBehavior = BottomSheetBehavior.from((View) contentView.getParent());
//        mBehavior.setPeekHeight(Utils.dipToPixels(getActContext(), 350));
        mBehavior.setState(BottomSheetBehavior.STATE_EXPANDED);
        mBehavior.setHideable(true);
//        MTextView moreTitleTxt = contentView.findViewById(R.id.moreTitleTxt);
        MTextView cancelTxt = contentView.findViewById(R.id.cancelTxt);

//        moreTitleTxt.setText(generalFunc.retrieveLangLBl("", "LBL_SEARCH_SERVICES"));
        EditText searchTxtView = contentView.findViewById(R.id.searchTxtView);

        searchTxtView.setHint(generalFunc.retrieveLangLBl("", "LBL_SEARCH_SERVICES"));

        cancelTxt.setText(generalFunc.retrieveLangLBl("", "LBL_CANCEL_TXT"));
        cancelTxt.setOnClickListener(v -> moreDialogSearchView.dismiss());

        MTextView noDataTxt = (MTextView) contentView.findViewById(R.id.noDataTxt);
        ...
        dataListRecyclerView_more.setVisibility(View.VISIBLE);
        ...
        searchTxtView.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void afterTextChanged(Editable searchTxt) {
                if (searchTxt.length() > 0) {
                    contentView.setMinimumHeight(Resources.getSystem().getDisplayMetrics().heightPixels);
                    mBehavior.setState(BottomSheetBehavior.STATE_EXPANDED);
                }
                searchCategoryList.clear();
                for (HashMap<String, String> searchHashMap : allMainCategoryList) {
                    if (searchHashMap.get("vCategory").toLowerCase().contains(searchTxtView.getText().toString().trim().toLowerCase())) {
                        searchCategoryList.add(searchHashMap);
                    }
                }
                if (searchCategoryList.size() > 0) {
                    dataListRecyclerView_more.setVisibility(View.VISIBLE);
                    ...
                    dataListRecyclerView_more.setVisibility(View.GONE);
                    noDataTxt.setVisibility(View.VISIBLE);
                }
                ufxCatAdapter.updateList(searchCategoryList);
            }
        });
